...
from datasets.data_generator_video import DataGenerator as DataGeneratorVid
# from models.irn import get_model, get_fused_model
from models import irn, cnn_grp
...
def load_args():
    ...
    ap.add_argument('-F','--fusion-irn',
        ...
        action='store_true')
    ap.add_argument('-C','--use-cnn',
        help="use cnn model",
        action='store_false') # reversed logic for facitily for now
        # action='store_true')
    ap.add_argument('-t','--temp-irn',
        help="use temporal_irn model",
        action='store_true')
    ap.add_argument('-T','--pool-window',
        help="temporal window to perform pool",
        type=int)
...
def get_irn_model(object_shape, data_kwargs, model_kwargs, fusion_irn, temp_irn,
                  indiv_out, output_size, indiv_output_size, fusion_kwargs={}):
    ...
    if not temp_irn:
        if not fusion_irn:
            model = irn.get_model(num_objs=num_joints, object_shape=object_shape, 
                output_size=output_size, num_dim=num_dim, overhead=overhead, 
                indiv_output_size=indiv_output_size, **model_kwargs)
        else:
            model = irn.get_fused_model(selected_joints, object_shape=object_shape,
                output_size=output_size, num_dim=num_dim, overhead=overhead,
...
def get_cnn_model(num_objs, data_kwargs, model_kwargs, fusion_cnn, temp_cnn,
                  indiv_out, output_size, indiv_output_size, fusion_kwargs={}):
    # timesteps = data_kwargs['timesteps']
    ...
    if not temp_cnn:
        if not fusion_cnn:
            model = cnn_grp.get_model(num_objs=num_objs, 
...
def predict(weights_path, model_kwargs, data_kwargs, dataset_name, 
        dataset_fold=None, batch_size=60, verbose=2, use_data_gen=True, 
        split='test', eval_metric='accuracy', temp_irn=False,
        fusion_irn=False, fusion_kwargs={}, append_log=True, use_cnn=False,
        pool_window=None):
    if verbose > 1:
        ...
        print("\t > Temporal pool window:", pool_window)
    ...
    if pool_window is not None:
        ## Ugly workaround, works only for CNN (possibly) + Frames
        data_kwargs['timesteps'] = pool_window
        batch_size = 1
    ...
    if use_data_gen:
        ...
        if not use_cnn:
            split_generator = DataGenerator(dataset_name, dataset_fold, split,
                batch_size=batch_size, reshuffle=False, 
            ...
            split_generator = DataGeneratorVid(
                dataset_name, dataset_fold, split,
                batch_size=batch_size, reshuffle=False, 
                shuffle_indiv_order=False,
                indiv_out=indiv_out, use_cw=False,
                **data_kwargs)
    ...
    if not use_cnn:
        ...
        model = get_irn_model(object_shape, data_kwargs, model_kwargs, 
                              fusion_irn, temp_irn, indiv_out, output_size, 
                              indiv_output_size, fusion_kwargs)
    else:
        num_objs = len(X)
        model = get_cnn_model(num_objs, data_kwargs, model_kwargs, 
                              fusion_irn, temp_irn, indiv_out, output_size, 
                              indiv_output_size, fusion_kwargs)
    ...
    for filepath in weights_path:
        ...
        if pool_window is not None:
            ## Testing, by using scores only from central frame
            # Y_pred = Y_pred[5::10]
            ## Worked fine!
            
            new_shape = (-1, pool_window) + Y_pred.shape[1:]
            
            use_gauss_weight = False
            print("use_gauss_weight:", use_gauss_weight)
            if use_gauss_weight:
                def gauss(n, sigma=1):
                ...
                Y_pred = np.average(Y_pred.reshape(new_shape), axis=1, 
                    weights=gauss(pool_window) )
            else:
                Y_pred = Y_pred.reshape(new_shape).mean(axis=1)
...
def exp_predict(dataset_name, fold, exp_name, exp_val, rerun=-1, 
                    criteria='eval', temp_irn=False, fusion_irn=False, 
                    save_preds=False, save_scrs=False, use_cnn=False,
                    noise_std=0, noise_dr=0,  split_fold=0,
                    **predict_kwargs):
    ...
    print("\t Criteria for chosing weights:", criteria)
    print("\t Using Temporal IRN:", temp_irn)
    print("\t Using Fusion IRN:", fusion_irn)
    print("\t Using CNN:", use_cnn)
    print("\t Saving Predictions:", save_preds)
    print("\t Saving Scores:", save_scrs)
    
    base_path = 'models/{}/hp_search/fold_{}/{}/{}'.format(
    # base_path = '../models/{}/hp_search/fold_{}/{}/{}'.format(
        dataset_name, fold, exp_name, exp_val)
    base_path = os.path.join(*base_path.split('/')) # Workaround for windows
    if use_cnn:
        base_path = base_path.replace('hp_search','hp_search_cnn')
    ...
    Y_pred, Y_true = predict(weights_path, model_kwargs, data_kwargs, 
            # dataset_name, fold, temp_irn=temp_irn, fusion_irn=fusion_irn,
            dataset_name, split_fold, temp_irn=temp_irn, fusion_irn=fusion_irn,
            use_cnn=use_cnn, fusion_kwargs=fusion_kwargs, 
            **predict_kwargs)
    
    #### WIP Suffix for saved files
    filename_suffix = ''
    pool_window = predict_kwargs.get('pool_window')
    if pool_window is not None:
        filename_suffix += '_pool_{}'.format(pool_window)
    if rerun >= 0:
        filename_suffix += '_r{}'.format(rerun)
...
def save_predictions(Y_pred, Y_true, out_path, indiv_out=False, suffix=''):
    if not indiv_out:
        ...
        np.save(os.path.join(out_path, 'y_true'+suffix+'.npy'), classes_true)
        np.save(os.path.join(out_path, 'y_pred'+suffix+'.npy'), classes_pred)
    else:
        ### TODO indiv_out
        Y_pred_grp, *Y_pred_inds = Y_pred
        Y_true_grp, *Y_true_inds = Y_true
        ...
        np.save(os.path.join(out_path, 'y_true_indiv'+suffix+'.npy'), classes_true)
        np.save(os.path.join(out_path, 'y_pred_indiv'+suffix+'.npy'), classes_pred)

def save_scores(Y, out_path, indiv_out=False, suffix=''):
    if not indiv_out:
        # invert one-hot and save as ints
        # classes_pred = np.argmax(Y_pred, axis=1).astype('int8')
        # np.save(os.path.join(out_path, 'y_pred_scores.npy'), classes_pred)
        
        np.save(os.path.join(out_path, 'y_pred_scores'+suffix+'.npy'), Y)
    else:
        ### TODO indiv_out
        Y_grp, *Y_inds = Y
        ...
        # classes_pred = np.argmax(Y_pred_grp, axis=1).astype('int8')
        # np.save(os.path.join(out_path, 'y_pred_scores.npy'), classes_pred)
        np.save(os.path.join(out_path, 'y_pred_scores'+suffix+'.npy'), Y_grp)
        
        # classes_pred = np.argmax(Y_pred_indiv, axis=1).astype('int8')
        # np.save(os.path.join(out_path, 'y_pred_scores_indiv.npy'), classes_pred)
        np.save(os.path.join(out_path, 'y_pred_scores_indiv'+suffix+'.npy'), Y_indiv)
